...
class VideoAnnotator(QMainWindow):
    ...
    def output_video(self):
        ...
        self.mask_colors = {
            ...
            2: np.array([0, 0, 255]),    # 红色
            3: np.array([255, 0, 0]),    # 蓝色
            4: np.array([255, 255, 0]),  # 黄色
            5: np.array([255, 0, 255]),  # 紫色
            6: np.array([0, 255, 255]),  # 青色
            7: np.array([0, 100, 100]),  # 深蓝色
            8: np.array([128, 255, 0]),  # 淡绿色
            9: np.array([128, 0, 255]),  # 淡紫色
            10: np.array([128, 128, 255]), # 淡蓝色
            11: np.array([145, 147, 241]),
            12: np.array([128, 128, 0]),  # 橄榄色
        }
        ...
        for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(self.inference_state):
            # if out_frame_idx % 10 == 0:
                # print(f"正在分割帧 {out_frame_idx}")
                # self.log_text.setText(f"状态：正在分割帧 {out_frame_idx}")
        ...
        for out_frame_idx in range(len(self.frame_names)):
            ...
            if out_frame_idx % 10 == 0:
                print(f"正在着色帧 {out_frame_idx}")
                self.log_text.setText(f"状态：正在着色帧 {out_frame_idx}")

            if out_frame_idx in video_segments:
                # 创建一个临时帧用于绘制掩码，避免直接修改原始帧
